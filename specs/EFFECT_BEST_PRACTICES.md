...
Keep this document focused on “global rules”. For detailed guidance on how we model services and how to choose between `Effect.Service`, `Context.Tag`, and `Layer.*`, see `specs/EFFECT_LAYERS.md`.

Alibio defaults:
- Prefer `Effect.Service` for app-level services.
- Avoid requirement leakage: service methods should usually be `Effect<_, _, never>`.

### Effect.Service (Preferred)

Use `Effect.Service` for application services. It combines the Tag and Layer into a single class definition and handles dependency composition automatically.

**Avoid requirement leakage**: service methods should generally return `Effect<Success, Error, never>`.

- Capture dependencies during service construction (initializer).
- Don’t require other services at call time *unless you intentionally need late-binding/strategy selection*. See `specs/EFFECT_LAYERS.md`.

Avoid `accessors: true` by default.

It can obscure dependency relationships and make it easier to accidentally introduce requirement leakage patterns. Prefer explicitly extracting the service (`const svc = yield* MyService`) and then calling its methods.

```typescript
import { Effect } from "effect"
import { SqlClient } from "@effect/sql"
import { PgLive } from "@alibio/persistence"
...
export class AccountService extends Effect.Service<AccountService>()("AccountService", {
  effect: Effect.gen(function* () {
    ...
    return {
      findById: (id: AccountId) => Effect.gen(function* () {
      ...
      create: (account: Account) => Effect.gen(function* () {
        // ...
      })
    }
  }),
  dependencies: [PgLive]  // Dependencies are composed automatically
}) {}

// Usage:
// - AccountService (the Tag) - yield* AccountService in Effect.gen
// - AccountService.Default - Layer with dependencies already provided
// - AccountService.DefaultWithoutDependencies - for testing with mocks
```

**Testing with Effect.Service:**

```typescript
// Option 1: Use DefaultWithoutDependencies and provide mock deps
const TestLayer = AccountService.DefaultWithoutDependencies.pipe(
  Layer.provide(MockSqlClientLive)
)

// Option 2: Mock the service directly
const mockService = new AccountService({
  findById: () => Effect.succeed(mockAccount),
  findAll: () => Effect.succeed([mockAccount]),
  create: (account) => Effect.succeed(account)
})

const program = myProgram.pipe(
  Effect.provideService(AccountService, mockService)
)
```

See [Effect.Service documentation](https://effect.website/docs/requirements-management/layers/#simplifying-service-definitions-with-effectservice) for more details.

### Context.Tag (When Needed)

Use `Context.Tag` directly when:
- Building library-style code where no sensible default implementation exists
- The “service” is inherently contextual (per-request/per-fiber/per-tenant values)
- You want to publish only the interface and force the caller to decide the implementation

If you find yourself inventing a fake `.Default` implementation just to satisfy `Effect.Service`, that’s usually a sign you actually want `Context.Tag`.

```typescript
import { Context } from "effect"

// Interface-only Tag - no default implementation
...
// Consumers must provide their own Layer
```

### Creating Layers - Use `Layer.effect` or `Layer.scoped`

When using `Context.Tag` directly:
...
**Layer.scoped** - When the service needs cleanup (subscriptions, background fibers, open handles, etc.):
